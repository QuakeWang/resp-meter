...
impl Case for Set {
    ...
    async fn run(&self, mut c: resp::Client) {
        if self.requests.key_range == 0 || self.requests.key_range >= 100_0000_0000 {
            println!(
                "Invalid key range: {}, must be in (0, 100_0000_0000)",
                self.requests.key_range
            );
            return;
        }
        if self.requests.value_size == 0 || self.requests.value_size >= 100_0000_0000 {
            println!(
                "Invalid value size: {}, must be in (0, 100_0000_0000)",
                self.requests.value_size
            );
            return;
        }
        ...
        let distribution = crate::case::DistributionEnum::from_str(
            &self.requests.key_distribution,
            self.requests.key_range,
        );
